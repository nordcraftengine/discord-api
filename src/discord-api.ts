import { ThreadsResponse, Threads, Messages, Users } from './types';
...
export const getAllTopics = async (env: Env, channelId?: string) => {
	...
	return channelId ? threadsData.threads.filter((t) => t.parent_id === channelId) : threadsData.threads;
};

export const getMessages = async (threads: { id: string; after?: string }[], env: Env) => {
	const messages = (
		await Promise.all(
...
export const getNewData = async (env: Env) => {
	...
	// Get the new topics
	const newTopics = allTopics.filter((thread) => !savedTopicIds.includes(thread.id));

	//Get the topics with new messages
	// const newMessagesTopics = allTopics.filter((thread) => !savedTopicLastMessageId.includes(thread.last_message_id));
	const newMessagesTopics = allTopics
		.map((thread) => {
			const lastSavedMessage = savedTopics?.find((t) => t.id === thread.id)?.last_message_id;
	...
	const savedUserIds = (await supabase.from('users').select('id')).data?.map((user) => user.id) ?? [];

	newMessages.forEach((message) => {
		if (!savedUserIds.includes(message.author.id) && !newUsers.find((user) => user.id === message.author.id)) {
			newUsers.push(message.author);
		}

		const newMentionUsers = message.mentions.filter((m) => !savedUserIds.includes(m.id) && !newUsers.find((user) => user.id === m.id));
